...
function Leaderboard() {
  ...
  useEffect(() => {
    async function fetch() {
      ...
      // Aggregate gameweek rows per participant
      const map = {}
      for (const row of data || []) {
        const id = row.participant_id
        if (!map[id]) map[id] = {
          name: row.participants?.name ?? '—',
          paid: row.participants?.paid ?? false,
          team_points: 0,
          player_points: 0,
          total_points: 0,
        }
  ...
  if (!rows.length) return (
    <EmptyState icon="🏆" message="No scores yet. The leaderboard will populate once matches kick off." />
  )
            ...
            <tr key={row.name} style={{ background: i % 2 ? C.stripe : C.white }}>
              <td style={{ ...S.td, textAlign: 'center' }}>
                <span style={{
                  ...
                  width: 28, height: 28,
                  borderRadius: '50%',
                  background: i < 3 ? podium[i] + '22' : 'transparent',
                  color: i < 3 ? podium[i] : C.muted,
                  fontFamily: "'Barlow Condensed', sans-serif",
                  fontWeight: 700,
                  fontSize: 15,
                }}>
              ...
              <td style={S.td}>
                <Badge colors={row.paid ? C.paid : C.free}>
                  {row.paid ? 'Paid' : 'Free'}
                </Badge>
              </td>
              <td style={{ ...S.td, textAlign: 'right', color: C.muted }}>
                {row.team_points.toFixed(1)}
              </td>
              <td style={{ ...S.td, textAlign: 'right', color: C.muted }}>
                {row.player_points.toFixed(1)}
              </td>
              <td style={{
                ...S.td,
                textAlign: 'right',
...
function TeamOwnership() {
    ...
    async function fetch() {
      ...
      setRows(
        Object.values(map)
          ...
          .map(r => ({ ...r, teams: r.teams.sort((a, b) => a.pool.localeCompare(b.pool)) }))
      )
      setLoading(false)
    }
    fetch()
  ...
  if (error)   return <EmptyState icon="⚠️" message={`Error: ${error}`} />
  if (!rows.length) return (
    <EmptyState icon="🌍" message="No team assignments yet. Run the draw to assign teams." />
  )
              ...
              <tr key={row.name} style={{ background: i % 2 ? C.stripe : C.white }}>
                ...
                {['A', 'B', 'C'].map(pool => (
                  <td key={pool} style={S.td}>
                    {byPool[pool].length ? (
                      byPool[pool].map(t => (
                        <Badge key={t.name} colors={poolColors[pool] || C.free}>
                          {t.name}{t.fifa ? ` · ${t.fifa}` : ''}
                        </Badge>
                      ))
                    ) : (
...
function WeeklyPicks() {
  const [picks, setPicks] = useState({})       // { gwId: [rows] }
  const [gwMeta, setGwMeta] = useState({})     // { gwId: label }
  ...
  useEffect(() => {
    async function fetch() {
      const [picksRes, gwRes] = await Promise.all([
        supabase
          ...
          .select(`
            gameweek_id,
            participants ( name ),
            players ( name, position, teams ( name, fifa_code ) )
          `),
        supabase
          .from('gameweeks')
          .select('*')
      ...
      // Build gameweek label map from whatever columns exist
      const meta = {}
      for (const gw of gwRes.data || []) {
        const label = gw.name || gw.number || gw.week_number || gw.id.slice(0, 6)
        meta[gw.id] = String(label)
      }
      setGwMeta(meta)
  ...
  const gwIds = Object.keys(picks)
  if (!gwIds.length) return (
    <EmptyState icon="⚽" message="No player picks yet. Picks will appear here once participants select their gameweek player." />
  )
  ...
  const posColors = {
    ...
    MF: { bg: '#fef3c7', text: '#92400e' },
    DF: { bg: '#dbeafe', text: '#1e40af' },
    GK: { bg: '#f3f4f6', text: '#374151' },
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
        {gwIds.map(gwId => {
          const label = gwMeta[gwId] || gwId.slice(0, 6)
          const active = gwId === activeGw
          return (
            <button
              key={gwId}
              onClick={() => setActiveGw(gwId)}
              style={{
                padding: '8px 18px',
                borderRadius: 8,
            ...
            {current.map((row, i) => {
              ...
              return (
                <tr key={i} style={{ background: i % 2 ? C.stripe : C.white }}>
                  ...
                  <td style={S.td}>
                    <Badge colors={posC}>{row.position}</Badge>
                  </td>
                  <td style={S.td}>
                    {row.team}
                    {row.fifa && <span style={{ color: C.muted, marginLeft: 6, fontSize: 12 }}>({row.fifa})</span>}
                  </td>
                </tr>
              )
            })}
